...
class ImageCode implements ImageCodeInterface
{
    /**
     * @var resource 验证码图片
     */
    ...
    public function initialize(array $config)
    {
        ...
        $this->fontSize = intval($this->width / floatval($this->number * 1.2));
    ...
    public function create()
    {
        ...
        for ($i = 0; $i < $this->number; $i++) {
            ...
            $codeNX += mt_rand($this->fontSize * 0.8, $this->fontSize * 1.1);
            ...
            imagettftext($this->image, $this->fontSize, mt_rand(-10, 20), $codeNX, $this->fontSize * 1.3, $color, $this->textFont, $code[$i]);
        }
    ...
    public function toBase64($key)
    {
        $file = APP_PUBLIC . '/upload/' . md5($key) . '.png';
        $this->save($file, 1);
        $base64Str = base64_encode(file_get_contents($file));
        unlink($file);
...
//        $increase = mt_rand(30, 255);
//
//        $red = abs(min(150, $red - $increase));
//        $green = abs(min(150, $green - $increase));
//        $blue = abs(min(150, $blue - $increase));

        return [mt_rand(1, 60), mt_rand(1, 60), mt_rand(1, 60)];
    }
}