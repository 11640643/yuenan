...
class ProblemAnswer extends Service
{
    protected function setModel()
    {
        $this->model = new \C\M\ProblemAnswer();
    }

    public function getStatusConfig()
    {
	$problems = $this->di['s_problem']->searchAll([],[],['id','title']);
	$category = [];
	foreach($problems as $problem){
		$category[$problem['id']] = $problem['title'];
	}
        return [
	    'category'=>$category
            /*'category' => [
    ...
    public function start($uid)
    {
        try {
            ...
            if ($config['is_open'] != 'Y' || $config['begin_time'] > $time || $time > $config['begin_time'] + $config['stop_min'] * 60) {
                throw new \Exception($this->translate['active_not_begin']);
            }
            ...
            if($anwVipNum < 1){
                if (!$this->di['s_funds']->add($uid, -1, 'anwser_num', 'anwser_num_sub', $this->translate['dati'], $cid, 'Y', $numArray[0], $numArray[1])) {
